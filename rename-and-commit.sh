...
echo "========================================"
echo "Git Rename and Commit Operation"
echo "========================================"
echo "Total files to rename: ${#renameMap[@]}"
echo ""

for oldName in "${!renameMap[@]}"; do
    newName=${renameMap[$oldName]}
    oldPath="images/$oldName"
    newPath="images/$newName"
    
    if [ -f "$oldPath" ]; then
        if git mv "$oldPath" "$newPath" 2>/dev/null; then
        ...
        echo "⊘ Skipped (not found): $oldName"
        ((skippedCount++))
    fi
done
...
echo "========================================"
echo "Rename Summary:"
echo "Success: $successCount | Failed: $failedCount | Skipped: $skippedCount"
echo "========================================"
...
if [ $stagedCount -gt 0 ]; then
    echo ""
    echo "Committing $stagedCount renamed files..."
...
- No changes to file content or generation logic"
    ...
    if [ $? -eq 0 ]; then
        ...
        echo ""
        echo "Next steps:"
        echo "  1. Review: git log --oneline -1"
        echo "  2. Push: git push origin <branch-name>"
    else
        echo "✗ Commit failed"
    fi
else
