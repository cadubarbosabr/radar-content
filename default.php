<?php
// =================================================================================
// ENGINE MARKDOWN PARA PHP (RADAR ELITE VERSION)
// =================================================================================
...
// Lógica de Leitura de Posts (Mantida)
function getPosts() {
    $files = glob("posts/*.md");
    $posts = [];
    ...
    usort($posts, function($a, $b) {
        // Tenta ordenar por data, fallback para criação do arquivo
...
<head>
        ...
        tailwind.config = {
            theme: {
                extend: {
                    ...
                    colors: {
                        radar: {
                            bg: '#0B0F19',       // Fundo Profundo
                            card: '#151B2B',     // Card Ligeiramente mais claro
                            border: '#2A3441',   // Bordas sutis
                            accent: '#6366f1',   // Indigo Primário
                            glow: '#818cf8',     // Brilho
    ...
    <style>
        ...
        /* TIPOGRAFIA DE LEITURA (PROSE CUSTOMIZATION) */
        ...
        .prose blockquote { border-left-color: #6366f1; background: rgba(99,102,241,0.05); color: #a5b4fc; font-style: normal; border-radius: 0 8px 8px 0; }
        ...
        .prose img { border-radius: 12px; border: 1px solid rgba(255,255,255,0.05); box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.3); }
    </style>
</head>
<body class="bg-radar-bg text-radar-text antialiased min-h-screen relative selection:bg-radar-accent selection:text-white overflow-x-hidden">
        ...
        <div class="border-b border-white/5 bg-black/20">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-10 flex items-center justify-end gap-6 text-xs font-mono tracking-wider uppercase">
                ...
                <?php foreach($site_config['external_links'] as $label => $link): ?>
                    <a href="<?php echo $link; ?>" target="_blank" class="text-radar-muted hover:text-radar-accent hover:shadow-[0_0_15px_rgba(99,102,241,0.5)] transition-all duration-300 flex items-center gap-1 group">
                        <?php echo $label; ?>
            ...
            <div class="flex flex-col md:flex-row items-center justify-between gap-6">
                ...
                <div class="flex items-center gap-3 group cursor-pointer" onclick="window.scrollTo(0,0)">
                    ...
                    <div class="flex flex-col">
                        <span class="text-xl font-display font-bold tracking-tight text-white leading-none">
                            RADAR
                        </span>
                        <span class="text-[10px] font-mono text-radar-accent uppercase tracking-[0.2em] leading-none mt-1">
                            Cadu Barbosa
                        </span>
                    </div>
                </div>
                ...
                <nav class="flex flex-wrap justify-center gap-2 bg-white/5 p-1 rounded-full backdrop-blur-sm border border-white/5">
                    <button onclick="filterPosts('all')" class="filter-btn px-5 py-2 rounded-full text-xs font-bold uppercase tracking-wide transition-all duration-300 bg-radar-accent text-white shadow-lg shadow-indigo-500/20" data-filter="all">
                        All Signals
                    </button>
                    <?php foreach ($categories as $cat): ?>
                    <button onclick="filterPosts('<?php echo $cat; ?>')" class="filter-btn px-5 py-2 rounded-full text-xs font-bold uppercase tracking-wide transition-all duration-300 text-radar-muted hover:text-white hover:bg-white/5 border border-transparent" data-filter="<?php echo $cat; ?>">
                        <?php echo $cat; ?>
                    </button>
                    <?php endforeach; ?>
                </nav>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-48 pb-32 relative z-10">
        ...
        <div class="mb-20 text-center max-w-3xl mx-auto space-y-6">
            ...
            <h1 class="text-4xl md:text-6xl font-display font-bold tracking-tight text-transparent bg-clip-text bg-gradient-to-b from-white via-white to-slate-400">
                Intelligence & Signals
            </h1>
            <p class="text-lg md:text-xl text-radar-muted font-light leading-relaxed max-w-2xl mx-auto">
                <?php echo $site_config['subtitle']; ?>
            </p>
        </div>
            ...
            <article class="post-card break-inside-avoid glass-card rounded-2xl overflow-hidden cursor-pointer group relative flex flex-col" 
                ...
                <div class="relative aspect-[16/10] overflow-hidden">
                    ...
                    <div class="absolute top-4 left-4">
                        <span class="px-3 py-1 bg-black/60 backdrop-blur-md border border-white/10 text-white text-[10px] font-bold font-mono uppercase tracking-wider rounded-md shadow-lg">
                ...
                <div class="p-8 flex-1 flex flex-col">
                    ...
                    <h2 class="text-2xl font-display font-bold text-white mb-4 leading-tight group-hover:text-radar-accent transition-colors">
                        <?php echo $post['title']; ?>
                    </h2>
                    ...
                    <div class="mt-auto pt-6 border-t border-white/5 flex justify-between items-center">
                        <span class="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-2 group-hover:gap-3 transition-all">
                            Ler Insight 
                            <svg class="w-4 h-4 text-radar-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"></path></svg>
                        </span>
                    </div>
                </div>
            </article>
    ...
    <footer class="border-t border-white/5 bg-black/40 py-12 backdrop-blur-md relative z-10">
        <div class="max-w-7xl mx-auto px-4 text-center">
            <p class="text-slate-600 text-xs font-mono uppercase tracking-widest">
                &copy; <?php echo date('Y'); ?> Radar Cadu Barbosa. <span class="text-indigo-900/50 mx-2">|</span> All Systems Nominal.
            </p>
        </div>
    </footer>
        ...
        <div class="fixed inset-0 overflow-y-auto">
            <div class="flex min-h-full items-center justify-center p-0 md:p-6 text-center">
                ...
                <div class="relative w-full max-w-4xl transform transition-all opacity-0 scale-95 duration-500" id="modal-panel">
                    ...
                    <div class="fixed top-0 left-0 w-full md:w-[calc(100%-3rem)] md:left-6 md:top-6 md:rounded-t-2xl h-1 z-50 bg-slate-800/50 overflow-hidden pointer-events-none">
                        <div id="read-progress" class="h-full bg-gradient-to-r from-indigo-500 to-purple-500 w-0 shadow-[0_0_10px_rgba(99,102,241,0.5)]"></div>
                    </div>

                    <button onclick="closeModal()" class="fixed top-6 right-6 z-50 p-3 rounded-full bg-black/20 text-slate-400 hover:text-white hover:bg-radar-accent hover:rotate-90 transition-all duration-300 border border-white/10 backdrop-blur-md group">
                    ...
                    <div class="bg-[#0f1420] text-left md:rounded-2xl shadow-2xl overflow-hidden border border-white/5 relative">
                        ...
                        <div class="modal-scroll max-h-[100vh] md:max-h-[90vh] overflow-y-auto custom-scroll">
                            ...
                            <div class="px-6 py-12 md:px-16 md:py-16 bg-[#0f1420]">
                                <div class="max-w-2xl mx-auto"> <div class="flex items-center gap-4 mb-12 pb-8 border-b border-white/5 opacity-0 translate-y-4 animate-[fadeUp_0.8s_0.4s_forwards]">
                                        ...
                                        <div>
                                            <p class="text-white font-bold text-sm">Cadu Barbosa</p>
                                            <p class="text-slate-500 text-xs">Curador & Estrategista</p>
                                        </div>
                                    </div>

                                    <div id="modal-body" class="prose prose-lg prose-invert max-w-none opacity-0 translate-y-4 animate-[fadeUp_0.8s_0.5s_forwards]">
                                        </div>
                                    ...
                                    <div class="mt-20 pt-10 border-t border-white/5 opacity-0 translate-y-4 animate-[fadeUp_0.8s_0.6s_forwards]">
                                        ...
                                        <div class="flex justify-center gap-4">
                                            <a id="share-whatsapp" target="_blank" class="p-3 rounded-full bg-white/5 text-slate-300 border border-white/5 hover:bg-[#25D366] hover:text-white hover:border-[#25D366] transition-all duration-300 hover:scale-110">
                                            ...
                                            <a id="share-linkedin" target="_blank" class="p-3 rounded-full bg-white/5 text-slate-300 border border-white/5 hover:bg-[#0077b5] hover:text-white hover:border-[#0077b5] transition-all duration-300 hover:scale-110">
                                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                                            </a>
                                            <a id="share-twitter" target="_blank" class="p-3 rounded-full bg-white/5 text-slate-300 border border-white/5 hover:bg-black hover:text-white hover:border-white/20 transition-all duration-300 hover:scale-110">
    ...
    <script>
        ...
        function openModal(post) {
            ...
            document.getElementById('modal-img').src = post.image || ''; // Fallback se não tiver imagem
            if(!post.image) document.getElementById('modal-img').style.display = 'none';
            else document.getElementById('modal-img').style.display = 'block';
