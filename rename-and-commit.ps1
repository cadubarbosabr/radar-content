# Git rename and commit script for image files
# Format B: YYYY-MM-DD(-HHMM)-descriptive-name.ext
...
Write-Host "Git Rename and Commit Operation" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "Total files to rename: $($renameMap.Count)`n" -ForegroundColor White

foreach ($oldName in $renameMap.Keys) {
    $newName = $renameMap[$oldName]
    $oldPath = Join-Path "images" $oldName
    $newPath = Join-Path "images" $newName
    
    if (Test-Path $oldPath) {
        try {
            $output = git mv "$oldPath" "$newPath" 2>&1
            if ($LASTEXITCODE -eq 0) {
                Write-Host "✓ $oldName → $newName" -ForegroundColor Green
                $successCount++
            } else {
                Write-Host "✗ Failed: $oldName - $output" -ForegroundColor Red
                $failedCount++
            }
        }
        catch {
        ...
        Write-Host "⊘ Skipped (not found): $oldName" -ForegroundColor Yellow
        $skippedCount++
    }
}
...
Write-Host "Success: $successCount | Failed: $failedCount | Skipped: $skippedCount" -ForegroundColor White
Write-Host "========================================" -ForegroundColor Cyan
...
if ($stagedCount -gt 0) {
    Write-Host "`nCommitting $stagedCount renamed files..." -ForegroundColor Cyan
    git commit -m "refactor: standardize image file naming to YYYY-MM-DD(-HHMM)-descriptive-name format

- Rename 66 image files to follow consistent chronological sorting pattern
- Format: YYYY-MM-DD(-HHMM)-descriptive-name.ext
- Ensures proper date/time based ordering in file explorer
- No changes to file content or generation logic"
    
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✓ Commit successful!" -ForegroundColor Green
        Write-Host "`nNext steps:" -ForegroundColor Cyan
        Write-Host "  1. Review: git log --oneline -1" -ForegroundColor White
        Write-Host "  2. Push: git push origin <branch-name>" -ForegroundColor White
    } else {
        Write-Host "✗ Commit failed" -ForegroundColor Red
    }
} else {
    Write-Host "`n⊘ No staged changes to commit" -ForegroundColor Yellow
}
